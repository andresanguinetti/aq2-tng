...
void TallyEndOfLevelTeamScores (void)
{
	...
	// Adjust rand() % 2 to N where N is the number of tng/round_end_N.wav files we want to randomize
	gi.sound(&g_edicts[0], CHAN_VOICE | CHAN_NO_PHS_ADD, gi.soundindex(va("tng/round_end_%i.wav", rand() % 2)), 1, ATTN_NORM, 0 );
